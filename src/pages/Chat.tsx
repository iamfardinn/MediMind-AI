                  ...
                  <div className="p-4 flex flex-col gap-3">
                    {mockMessages.map((msg, i) => (
                      <motion.div key={i}
                        ...
                        <div className={`max-w-[82%] px-3.5 py-2.5 text-[12.5px] leading-relaxed ${msg.role === 'user'
          ...
          <AnimatePresence initial={false}>
            {messages.map((msg) => {
              ...
              return (
                ...
                >
                  {/* Avatar */}
                  <div className={`w-7 h-7 rounded-full shrink-0 flex items-center justify-center shadow-md ${isUser ? 'bg-linear-to-br from-indigo-500 to-purple-600' : 'bg-linear-to-br from-sky-500 to-cyan-500'
                    }`}>
                    {isUser ? <User className="w-3.5 h-3.5 text-white" /> : <Bot className="w-3.5 h-3.5 text-white" />}
                  </div>

                  {/* Bubble */}
                  <div className={`inline-block max-w-[min(82%,640px)] min-w-0 shrink px-4 py-2.5 text-[0.825rem] leading-relaxed wrap-break-word text-slate-100 ${isUser
